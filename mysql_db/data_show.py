...
def cate_rel_show(sql):
    conn = pymysql.connect(host='localhost', port=3306, user='root', passwd='123456', db='database1')  # db:表示数据库名称
...
def operation(sql, item):
    conn = pymysql.connect(host='127.0.0.1', port=3306, user='root1', passwd='Root_12root', db='database1')  # db:表示数据库名称
